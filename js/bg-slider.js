...
var skrollerDynamic = function () {
    ...
    var windowHeight = $(window).height(), sectionHomeHeight = $('.section-home').height(), sectionMainHeight = $('.section-main').height(), sectionBlackHeight = $('.section-black').height(), sectionWhiteHeight = $('.section-white').height(), footerHeight = $('footer').height(), sectionAboutVideoHeight = $('.top-video').height(), sectionAboutTrustHeight = $('.about-trust').height();
    ...
    if (skrollrInst == undefined) {
        skrollrInst = skrollr.init({forceHeight: false});
    } else {
        skrollrInst.refresh();
    }
}
...
jQuery.extend(verge);
var desktop = true, tablet = false, mobile = false, tabletPortrait = false;
...
$(function () {
    ...
    $(window).load(function () {
        setTimeout(function () {
            ...
            setTimeout(function () {
                ...
                if (block.length) {
                    $('body, html').animate({scrollTop: block.offset().top - 20}, 1000);
                }
            }, 400);
        }, 1500);
    });
    ...
    var renderTrace = function (path) {
        path.style.transition = path.style.WebkitTransition = 'stroke-dashoffset 2s ease-in-out, fill-opacity 2s ease 1.5s';
        path.style.fillOpacity = '1';
        path.style.strokeDashoffset = '0';
    };
    var renderTraceLogo = function (path) {
        path.style.transition = path.style.WebkitTransition = 'stroke-dashoffset 10s ease-in-out, fill-opacity 2s ease 2s';
        path.style.fillOpacity = '1';
        path.style.strokeDashoffset = '0';
    };
    if (preloader.length) {
    ...
    var backgroundAnimation = function () {
        ...
        function scrollCheck() {
            if ($(document).scrollTop() > height / 2)animateHeader = true; else animateHeader = false;
        }
    ...
    if (!touch) {
        ...
        $('.articles_ii').mCustomScrollbar({scrollInertia: 1000, scrollEasing: "ease"});
        if ($('#bg-animation').length) {
            backgroundAnimation();
        }
        ...
        $('.btn-next').on('click', function (e) {
            e.preventDefault();
            $('body, html').animate({scrollTop: $('.section-main').offset().top}, 1000);
        });
    }
...
var isotopeResizeSizeCalc = function (box) {
    $(window).resize(function () {
        ...
        box = box.isotope({masonry: {columnWidth: columnWidth}});
    }).resize();
}
$(function () {
    var topVideo = $('.top-video');
        ...
        if (topVideo.length) {
            $(window).on('scroll', function () {
                ...
                videoMuteScroll = setTimeout(function () {
                    var scrollTop = $(window).scrollTop(), videoOffset = topVideo.offset().top;
    ...
    var videoControls = $('.video-controls'), videoBox = $('.top-video video');
    ...
    $('.contact-form_i input[type="text"], .contact-form_i textarea, .subscribe__input input').add('.js-price-calc-form').find('input[type="text"]').on('focus', function () {
        var el = $(this);
    ...
    $('.js-open-calc-form').on('click', function (e) {
        ...
        $calcPriceForm.css('bottom', $('.footer').height()).css('opacity', 1)
        if (window.innerWidth > 1024) {
            $calcPriceForm.css('left', mainNavWidth).css('width', (window.innerWidth - mainNavWidth - 100));
        } else {
            $calcPriceForm.css('left', (window.innerWidth - contentWidth) / 2).css('width', contentWidth);
        }
    ...
    if ($('.sub-page_cookies').length) {
        Cookies.set('video-played-' + $('.top-video source').attr('src'), true, {
            expires: 1,
